...
export default function UserForm(props: Props) {
  const { onClose } = props;
  const { refetch } = useUserListQuery({ enabled: false });
      ...
      <FormProvider methods={methods} onSubmit={methods.handleSubmit(handleCreate)}>
        <Grid container padding={2} gap={2}>
          ...
          <Grid item xs={12} pt={2}>
            <Button variant="contained" type="submit" disabled={!methods.formState.isValid}>
              {t('button.create')}
            </Button>
            <Button sx={{ ml: 2 }} variant="contained" color="inherit" onClick={onClose}>
              {t('button.cancel')}
            </Button>
          </Grid>
        </Grid>
